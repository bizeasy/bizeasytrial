...
if(UtilValidate.isNotEmpty(parameters.productStoreId)){
	Cond = EntityCondition.makeCondition(
		[
		 EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, parameters.productStoreId),
		 EntityCondition.makeCondition("facilityId", EntityOperator.EQUALS, parameters.facilityId)
		],
		EntityOperator.AND)
	prodStoreFacs = from("ProductStoreFacility").where(Cond).queryList()
	if(UtilValidate.isEmpty(prodStoreFacs)){
		return;
	}else{
...
scrapLocations = from("FacilityLocation").where("facilityId", facilityId, "locationTypeEnumId", "FLT_SCRAP").queryList()
Debug.log("scrapLocations ========================="+scrapLocations);
context.scrapLocations = scrapLocations

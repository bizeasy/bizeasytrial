...
public class TaxAuthorityServices {
        ...
        try {
            ...
            if ("Y".equals(productStore.getString("showPricesWithVatTax"))) {
                ...
                for (GenericValue taxAdjustment : taxAdustmentList) {
                    if ("SALES_TAX".equals(taxAdjustment.getString("orderAdjustmentTypeId"))  || "IGST".equals(taxAdjustment.getString("orderAdjustmentTypeId")) || "SGST".equals(taxAdjustment.getString("orderAdjustmentTypeId")) || "CGST".equals(taxAdjustment.getString("orderAdjustmentTypeId"))) {
    ...
    public static Map<String, Object> rateProductTaxCalc(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        Debug.log("payToPartyId ==============="+payToPartyId);
        Debug.log("facilityId ==============="+facilityId);
        Debug.log("orderType ==============="+orderType);
        try {
        ...
        Debug.log("enableGST ================="+productStore.getString("enableGST"));
        Debug.log("enableOriginFacilityTax ================="+productStore.getString("enableOriginFacilityTax"));
        Debug.log("facility ================="+facility);
        String taxApplicability = null;
        if(UtilValidate.isNotEmpty( productStore.get("enableGST")) && productStore.getString("enableGST").equals("Y") ){
        	//enableOriginFacilityTax
            String destinationStateGeoId = null;
            String originFacilityStateGeoId = null;
            destinationStateGeoId = shippingAddress.getString("stateProvinceGeoId");
            
            if(UtilValidate.isNotEmpty(orderType) && orderType.equals("PURCHASE_ORDER")){
            	
                try {
                	List<GenericValue> allPartyContactMechs = EntityQuery.use(delegator).from("PartyContactMechPurpose").where("partyId", payToPartyId, "contactMechPurposeTypeId", "GENERAL_LOCATION").filterByDate().queryList();
                	Debug.log("allPartyContactMechs ================="+allPartyContactMechs);
                	
                	if(UtilValidate.isNotEmpty(allPartyContactMechs)){
                    	GenericValue contactMech = (GenericValue) allPartyContactMechs.get(0);
                    	GenericValue supplierAddress = EntityQuery.use(delegator).from("PostalAddress").where("contactMechId", contactMech.get("contactMechId")).queryOne();
                    	originFacilityStateGeoId = supplierAddress.getString("stateProvinceGeoId");
                    	Debug.log("originFacilityStateGeoId ================="+originFacilityStateGeoId);
                    }
                } catch (GenericEntityException e) {
            	...
            	Debug.log("destinationStateGeoId ================="+destinationStateGeoId);
                if(UtilValidate.isNotEmpty( productStore.get("enableOriginFacilityTax")) && productStore.getString("enableOriginFacilityTax").equals("Y") ){
                	if (facility != null) {
                        // if there is no shippingAddress and there is a facility it means it is a face-to-face sale so get facility's address
                        try {
                            GenericValue facilityContactMech = ContactMechWorker.getFacilityContactMechByPurpose(delegator, facilityId, UtilMisc.toList("SHIP_ORIG_LOCATION", "PRIMARY_LOCATION"));
                            ...
                            if (facilityContactMech != null) {
                            	GenericValue originFacilityAddress = EntityQuery.use(delegator).from("PostalAddress").where("contactMechId", facilityContactMech.get("contactMechId")).queryOne();
                            	originFacilityStateGeoId = originFacilityAddress.getString("stateProvinceGeoId");
                            	Debug.log("originFacilityStateGeoId ================="+originFacilityStateGeoId);
                            }
                        } catch (GenericEntityException e) {
                            Debug.logError(e, "Data error getting tax settings: " + e.toString(), module);
                	...
                	Debug.log("originFacilityStateGeoId =======2222=========="+originFacilityStateGeoId);
                }
                else{
                	if(UtilValidate.isNotEmpty( productStore.get("vatTaxAuthGeoId")) ){
                		originFacilityStateGeoId = productStore.getString("vatTaxAuthGeoId");
                		Debug.log("originFacilityStateGeoId ======== 222 ========="+originFacilityStateGeoId);
                		
                	}
                }
            }
            
            if(destinationStateGeoId.equals(originFacilityStateGeoId)){
        		taxApplicability = "INTRA-STATE";
        	}
        	else{
        		taxApplicability = "INTER-STATE";
        	}
        	
        }
        Debug.log("taxApplicability ========== 2111======="+taxApplicability);
        ...
        for (int i = 0; i < itemProductList.size(); i++) {
            ...
            if (shippingAddress != null) {
                taxList = getTaxAdjustments(delegator, product, productStore, payToPartyId, billToPartyId, taxAuthoritySet, itemPrice, itemQuantity, itemAmount, shippingAmount, ZERO_BASE, taxApplicability);
            }
        ...
        if (orderShippingAmount != null && orderShippingAmount.compareTo(BigDecimal.ZERO) > 0) {
            List<GenericValue> taxList = getTaxAdjustments(delegator, null, productStore, payToPartyId, billToPartyId, taxAuthoritySet, ZERO_BASE, ZERO_BASE, ZERO_BASE, orderShippingAmount, ZERO_BASE, taxApplicability);
            orderAdjustments.addAll(taxList);
        }
        if (orderPromotionsAmount != null && orderPromotionsAmount.compareTo(BigDecimal.ZERO) != 0) {
            List<GenericValue> taxList = getTaxAdjustments(delegator, null, productStore, payToPartyId, billToPartyId, taxAuthoritySet, ZERO_BASE, ZERO_BASE, ZERO_BASE, ZERO_BASE, orderPromotionsAmount, taxApplicability);
            orderAdjustments.addAll(taxList);
        }
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore, 
            ...
            BigDecimal shippingAmount, BigDecimal orderPromotionsAmount, String taxApplicability) {
        Timestamp nowTimestamp = UtilDateTime.nowTimestamp();
        List<GenericValue> adjustments = new LinkedList<GenericValue>();
        Debug.log("taxApplicability ================="+taxApplicability);
        ...
        Debug.log("productStore ====================="+productStore);
        String priceIncTax = "N";
        // store expr
        EntityCondition storeCond = null;
        if (productStore != null) {
        	
        	if(UtilValidate.isNotEmpty(productStore.get("prodPriceIncTax"))){
        		priceIncTax = (String) productStore.get("prodPriceIncTax");
        	}
        ...
        Debug.log("taxAuthoritySet =============="+taxAuthoritySet);
        ...
        Debug.log("taxAuthoritiesCond =============="+taxAuthoritiesCond);
        ...
        try {
            ...
            if(UtilValidate.isNotEmpty(taxApplicability)){
            	mainExprs.add(EntityCondition.makeCondition("taxApplicabilityEnumId", EntityOperator.EQUALS, taxApplicability));
            }
            
            
            EntityCondition mainCondition = EntityCondition.makeCondition(mainExprs, EntityOperator.AND);
            
            Debug.log("mainCondition =============="+mainCondition);
            ...
            Debug.log("lookupList =============="+lookupList);
            ...
            for (GenericValue taxAuthorityRateProduct : lookupList) {
            	Debug.log("taxAuthorityRateProduct ============="+taxAuthorityRateProduct);
            	String taxAuthorityRateTypeId = (String) taxAuthorityRateProduct.get("taxAuthorityRateTypeId");
            	
            	Debug.log("taxAuthorityRateTypeId ============="+taxAuthorityRateTypeId);
                ...
                if(taxAuthorityRateTypeId.equals("IGST") || taxAuthorityRateTypeId.equals("CGST") || taxAuthorityRateTypeId.equals("SGST") ){
                	taxAdjValue.set("orderAdjustmentTypeId", taxAuthorityRateTypeId);
                }
                else{
                ...
                if (productPrice != null && "Y".equals(productPrice.getString("taxInPrice")) && itemQuantity != BigDecimal.ZERO) {
                	...
                	Debug.log("taxAdjValue ============="+taxAdjValue);
                    ...
                    Debug.log("taxAmountIncludedInFullPrice ============="+taxAmountIncludedInFullPrice);
                    ...
                    Debug.log("netItemPrice ============="+netItemPrice);
                	...
                	Debug.log("taxAdjValue ====== 1111 ======="+taxAdjValue);
    ...
    private static List<GenericValue> getTaxAdjustments(Delegator delegator, GenericValue product, GenericValue productStore, 
        ...
        try {
            ...
            for (GenericValue taxAuthorityRateProduct : lookupList) {
            	...
            	Debug.log("taxAuthorityRateProduct.taxAuthorityRateTypeId ============="+taxAuthorityRateProduct.get("taxAuthorityRateTypeId"));
                BigDecimal taxRate = taxAuthorityRateProduct.get("taxPercentage") != null ? taxAuthorityRateProduct.getBigDecimal("taxPercentage") : ZERO_BASE;
                BigDecimal taxable = ZERO_BASE;

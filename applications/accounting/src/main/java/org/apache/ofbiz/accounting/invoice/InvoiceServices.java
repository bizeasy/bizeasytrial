    ...
    public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
        ...
        try {
            ...
            for (GenericValue currentValue : billItems) {
                ...
                List<EntityExpr> adjExprs = UtilMisc.toList(EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, orderItem.getString("orderId")),
                        EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, orderItem.getString("orderItemSeqId")),
                        EntityCondition.makeCondition("orderAdjustmentTypeId", EntityOperator.IN, UtilMisc.toList("VAT_TAX","IGST","CGST","SGST")),
                        EntityCondition.makeCondition("amountAlreadyIncluded", EntityOperator.GREATER_THAN, BigDecimal.ZERO));
                EntityCondition adjAndCondition = EntityCondition.makeCondition(adjExprs, EntityOperator.AND);
                List<GenericValue> orderAdjustmentTaxList =  delegator.findList("OrderAdjustment", adjAndCondition, null, null, null, false);
                Debug.log("orderAdjustmentTaxList ====================="+orderAdjustmentTaxList);
                
                Debug.log("billingAmount ====================="+billingAmount);
                for(int i=0; i<orderAdjustmentTaxList.size(); i++){
                	GenericValue taxAdjustment = orderAdjustmentTaxList.get(i);
                	if (UtilValidate.isNotEmpty(taxAdjustment) && (taxAdjustment.getBigDecimal("amount").signum() == 0) && UtilValidate.isNotEmpty(taxAdjustment.getBigDecimal("amountAlreadyIncluded")) && taxAdjustment.getBigDecimal("amountAlreadyIncluded").signum() != 0) {
                		billingAmount = billingAmount.subtract( (taxAdjustment.getBigDecimal("amountAlreadyIncluded")).divide(billingQuantity)  );
                		Debug.log("billingAmount =======222=============="+billingAmount);
                		//BigDecimal sourcePercentageTotal = taxAdjustment.getBigDecimal("sourcePercentage").add(new BigDecimal(100));
                        ...
                        //amountAlreadyIncluded.add(orderItem.getBigDecimal("unitPrice").subtract(billingAmount));
                	} 
                }
                ...
                Debug.log("createInvoiceItemContext =======222=============="+createInvoiceItemContext);
                ...
                Debug.log("itemAdjustments =============="+itemAdjustments);
...
//                    }
                    ...
                    Boolean isTaxIncludedInPrice = ( (adj.getString("orderAdjustmentTypeId").equals("VAT_TAX")) || (adj.getString("orderAdjustmentTypeId").equals("IGST")) || (adj.getString("orderAdjustmentTypeId").equals("CGST")) || (adj.getString("orderAdjustmentTypeId").equals("SGST"))) && UtilValidate.isNotEmpty(adj.getBigDecimal("amountAlreadyIncluded")) && adj.getBigDecimal("amountAlreadyIncluded").signum() != 0;
                    Debug.log("isTaxIncludedInPrice =============="+isTaxIncludedInPrice);
                    ...
                    if (adjAlreadyInvoicedAmount.abs().compareTo(adj.getBigDecimal("amount").setScale(invoiceTypeDecimals, ROUNDING).abs()) > 0) {
                    	Debug.log("============== equal already adjusted ==============");
                        continue;
                    }
                        ...
                        if (adj.get("amount") != null) {
                                if("PROMOTION_ADJUSTMENT".equals(adj.getString("orderAdjustmentTypeId")) && adj.get("productPromoId") != null) {
                                    ...
                                    if (isTaxIncludedInPrice) {
                                    	...
                                    	if(UtilValidate.isNotEmpty(adj.get("amountAlreadyIncluded"))){
                                    		amount = adj.getBigDecimal("amountAlreadyIncluded");
                                    	}
                                    	else{
                                    		BigDecimal priceWithTax = originalOrderItem.getBigDecimal("unitPrice");
                                        	BigDecimal sourcePercentageTotal = adj.getBigDecimal("sourcePercentage").add(new BigDecimal(100));
                                        	BigDecimal priceWithoutTax = orderItem.getBigDecimal("unitPrice").divide(sourcePercentageTotal, 100, ROUNDING).multiply(new BigDecimal(100)).setScale(invoiceTypeDecimals, ROUNDING);
                                        	Debug.log("priceWithoutTax =============="+priceWithoutTax);
                                        	amount = priceWithTax.subtract(priceWithoutTax);
                    ...
                    if (amount.signum() != 0) {
                        ...
                        Debug.log("createInvoiceItemAdjContext =============="+createInvoiceItemAdjContext);
